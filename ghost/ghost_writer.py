...
def extract_front_matter_field(markdown, field):
    # This regex is now more crucial and should work correctly.
...
def save_post(title, content):
    ...
    The 'content' is now expected to be the full, valid post content
    including the front matter, as generated by the AI.
    """
...
def parse_response(response_text):
    """
    Parses the AI's response to separate the blog post from optional
    system and generation prompt updates.
    """
    # The post is everything before the optional prompt update tags.
    # We split the text by the first occurrence of a prompt tag.
    parts = re.split(r"(\\[(SYSTEM_PROMPT|GENERATION_PROMPT)\\].*)", response_text, maxsplit=1, flags=re.DOTALL | re.IGNORECASE)
    post = parts[0].strip()

    # The rest of the logic searches the original text for the prompt tags.
...
def generate_and_reflect(prior_context, openai):
    """
    Generates the main prompt for the AI, asking it to create a full blog post
    with valid Jekyll front matter.
    """
    ...
    meta_prompt = (
        ...
        "If you choose to rewrite them, provide the complete new text for the prompt you wish to change *after* the blog post, using the specified tags.\n\n"
