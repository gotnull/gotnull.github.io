...
STATE_FILE = "ghost/.agi_state.json"
...
def random_post_time_for_today():
    """Generate random datetime for today with hour (0-23) and minute (0-59)."""
    now = datetime.now()
...
def parse_response(response_text):
    parts = re.split(r"(\[SYSTEM_PROMPT\].*?\[/SYSTEM_PROMPT\]|\[GENERATION_PROMPT\].*?\[/GENERATION_PROMPT\])", response_text, maxsplit=1, flags=re.DOTALL | re.IGNORECASE)
    post = parts[0].strip()
    system_prompt = None
    ...
    if generation_prompt_match:
        generation_prompt = generation_prompt_match.group(1).strip()
...
def main():
    ...
    # Check post_created_date in state
    post_created_date = state.get("post_created_date")
    if post_created_date == now.strftime("%Y-%m-%d"):
        print("Post already created today. Exiting.")
        return

    # Check or set target_time in state
    target_time_str = state.get("target_time")
    target_time = None
    if target_time_str:
        target_time = datetime.fromisoformat(target_time_str)
        # If target_time is not today, reset
        if target_time.date() != now.date():
            target_time = None

    if not target_time:
        target_time = random_post_time_for_today()
        state["target_time"] = target_time.isoformat()
        save_state(state)
        print(f"New target post time set for today: {target_time.strftime('%H:%M')}")

    if now < target_time:
        print(f"Current time {now.strftime('%H:%M')} is before target time {target_time.strftime('%H:%M')}. Exiting.")
        return

    # Time reached, proceed to generate post
    ...
    # Mark post as created today and clear target_time for next day
    state["post_created_date"] = now.strftime("%Y-%m-%d")
